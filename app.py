...
if st.session_state.config_loaded and st.session_state.config_data:
    ...
    st.header("2. Arzthelferinnen konfigurrieren")
    ...
    for h in selected_helpers:
        with st.expander(h):
            ...
            st.session_state.helpers_cfg[h] = {
                'max_hours': max_h, 'times': times, 'areas': areas
            }
    ...
    if st.button("Plan erstellen"):
        ...
        # Demand-Matrix aus Konfiguration
            ...
            for s in schifts:
                for bereich, cfg in st.session_state.bereiche_cfg.items():
                    if s in cfg['shifts'][d]:
                        ...
                        if bereich.startswith('Rezeption') and rezeption_prioritaet in candidates:
        ...
        if plan:
            ...
            df_pivot = (
                df_plan.pivot(index='Slot', columns='Bereich', values='Helferin')
            ...
            # Highlighting für dataframe in Streamlit
            def highlight(data):
            ...
            st.dataframe(df_pivot.style.apply(highlight, axis=None),
